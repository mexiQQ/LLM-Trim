...
def main(args):
    ...
    #         'wikitext2', 
    #         # 'ptb'
    #     ], 1024, batch_size=64, device=args.eval_device)
    # with torch.no_grad():
    # ppl = PPLMse2(model, model, tokenizer, [
    #     'wikitext2', 
    #     # 'ptb'
    ...
    if args.block_wise:
        kwargs = {
            ...
            "channel_groups": {},
            "consecutive_groups": {
                layer.attn.q_proj: layer.attn.head_dim for layer in model.transformer.h
            },
            "customized_pruners": {
            },
            "root_module_types": None, 
            # "root_instances": [model.transformer.h[i].mlp.up_proj for i in range(args.block_mlp_layer_start, args.block_mlp_layer_end)]
            # "root_instances": [model.transformer.h[i].attn.q_proj for i in range(args.block_attention_layer_start, args.block_attention_layer_end)]
            "root_instances": [model.transformer.h[i].attn.q_proj for i in range(args.block_attention_layer_start, args.block_attention_layer_end)] +
                              [model.transformer.h[i].mlp.up_proj for i in range(args.block_mlp_layer_start, args.block_mlp_layer_end)]
        }
        ...
        for i in range(args.iterative_steps):
            ...
            for layer in model.transformer.h:
                # layer.attn.q_num_heads = layer.attn.q_proj.weight.data.shape[0] // layer.attn.q_head_dim
                # layer.attn.k_num_heads = layer.attn.k_proj.weight.data.shape[0] // layer.attn.k_head_dim
                # layer.attn.v_num_heads = layer.attn.v_proj.weight.data.shape[0] // layer.attn.v_head_dim
                # layer.attn.num_heads = layer.attn.v_proj.weight.data.shape[0] // layer.attn.v_head_dim

                layer.attn.q_head_dim = layer.attn.q_proj.weight.data.shape[0] // layer.attn.q_num_heads
                layer.attn.k_head_dim = layer.attn.k_proj.weight.data.shape[0] // layer.attn.k_num_heads
                layer.attn.v_head_dim = layer.attn.v_proj.weight.data.shape[0] // layer.attn.v_num_heads
                layer.attn.head_dim = layer.attn.v_proj.weight.data.shape[0] // layer.attn.v_num_heads
    ...
    with torch.no_grad():
        ...
        #     'wikitext2', 
        #     'ptb'
        # ], 128, batch_size= 8, device=args.eval_device)
