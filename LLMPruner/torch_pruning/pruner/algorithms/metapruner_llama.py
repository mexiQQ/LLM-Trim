                    ...
                    if False: 
                        for dep, idx in group:
                            ...
                            if "q_proj" in layer_name:
                                q_weight = layer.weight.view(32, 128, 4096)
                            elif "k_proj" in layer_name:
                                k_weight = layer.weight.view(32, 128, 4096)
                        ...
                        if kq_mode == 1:
                            ...
                            k_list = []
                            q_list = []
                            K = (current_channels - n_pruned) // 32
                            for i in range(32):
                                ...
                                k_list.append(new_k)
                                q_list.append(new_q)
                            new_k = torch.stack(k_list).view(-1, 4096)
                            new_q = torch.stack(q_list).view(-1, 4096)
                            ...
                            new_k = new_k_batch.transpose(-2, -1).reshape(-1, 4096)
                            new_q = new_q_batch.reshape(-1, 4096)
                            print("new_k shape:", new_k.shape)
                            print("new_q shape:", new_q.shape)
                        elif kq_mode == 5:
                            K = (current_channels - n_pruned) // 32
                            ...
                            new_k = new_k_batch.reshape(-1, 4096)
                            new_q = new_q_batch.reshape(-1, 4096)
